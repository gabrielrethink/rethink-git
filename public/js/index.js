    ...
    users.map(async (user) => {
        ...
        cards_div.innerHTML += `
            <div class="card">
                ...
                <p class="name">${user.name ? `<p>${user.name}</p>` : `<p>${user.login}</p>`}</p>
                <p class="company">${user.company ? `<small>${user.company}</small>` : ""}</p>
                <div class="details">
                    <div class="projects">
...
const redirect = (login) => {
    window.location.href = `./projects.html?login=${login}`;
}
