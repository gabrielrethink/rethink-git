const projects_div = document.querySelector(".cardsProjects");

const user = new URLSearchParams(window.location.search).get("user");

const onClickFunc = (fullName) =>{
    console.log(fullName);
    window.location.href=`/public/projectShow.html?user=${fullName}`;
}

let template="";
const gitGetProjects = async (user) => {
    
    const projects = await fetch(`https://api.github.com/users/${user}/repos`).then(res => res.json());
    const language = await fetch(`http://localhost:3000/languages`).then(res => res.json());
    projects.forEach(project => {
        template += `
            <div class="cardProjects" onclick="onClickFunc('${project.full_name}')">
                <div class="navCardsProjects">
                    <p class="titleProjectCards">${project.name.length > 8 ? project.name.slice(0,8) + '...' : project.name}</p>
                    <p class="visibilityProject">${project.visibility}</p>
                </div>
                <p class="textProjectCard">${project.description != null && project.description?.length > 69 ? project.description.slice(0, 69) + '...' : project.description != null ? project.description : ""}</p>
                <div class="footerProjectCard">
                    <div class="cardLanguageItens">
                        <div class="cardLanguageColor" style="background: ${project.language ? language[project.language].color : ""}"></div>
                        <p class="cardlanguage">${project.language ? project.language : ""}</p>
                    </div>
                    <div class="detailsCardProject">
                        <div class="detailCardProject">
                            <svg class="svgDetailCP" width="15" height="15" viewBox="0 0 15 15" fill="none" xmlns="http://www.w3.org/2000/svg">
                            ...
                            <p class="detailNumber">${project.forks_count}</p>
                        </div>
                        <div class="detailCardProject">
                            <svg class="svgDetailCP" width="17" height="17" viewBox="0 0 17 17" fill="none" xmlns="http://www.w3.org/2000/svg">
                            ...
                            <p class="detailNumber">${project.stargazers_count}</p>
                        </div>
                    </div>
                </div>
            </div> 
    ...
    projects_div.innerHTML = template;
};

window.addEventListener("DOMContentLoaded", () =>gitGetProjects(user));