...
const cardsProject_div = document.querySelector('.cards-project');


const getProjects = async () =>{
    const userName = 'carolinavaleriano';
    const token = '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';
    const projects = await fetch(`https://api.github.com/users/${login}/repos`, {headers: { Authorization: "Basic" + btoa(`${userName}:${token}`)}}).then((res) => res.json());
    const colors = await fetch(`http://localhost:3000/colors`).then((response) => response.json());
    
    projects.map(async (project) => {
        cardsProject_div.innerHTML += `
            <div class="card-project" onclick="redirect('${project.full_name}')">
                <div class="header">
                    <p class="project">${
                        project.name.length > 12 ? project.name.slice(0,12) + '...' : project.name
                    }</p>
                    <p class="visibility">${project.visibility}</p>
                </div>
                <p class="abstract">${project.description ? project.description : ''}</p>
                <div class="footer-project">
                    <div class="language">
                        <div class="color" style="background:${project.language ? colors[project.language].color : ''}"></div>
                    ...
                    <div class="icons-project">
                        <div class="icones">
                            ...
                            <p class="number">${project.forks_count}</p>
                        </div>
                        <div class="icones">
                            ...
                            <p class="number">${project.stargazers_count}</p>
                        </div>
                    </div>
                </div>
            </div>
...
const redirect = (project) => {
    window.location.href = `./projectShow.html?project=${project}`;
}
